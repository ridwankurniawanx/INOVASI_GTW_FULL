#!/usr/bin/env python3
# v20.py - Rilis Final dengan Kolom Signal Name
...
def find_timestamp_ms(data):
    if isinstance(data, dict):
        if 't' in data and isinstance(data['t'], (int, float)):
            return data['t']
        for val in data.values():
            result = find_timestamp_ms(val)
            if result is not None:
                return result
    if isinstance(data, list):
        for item in data:
            result = find_timestamp_ms(item)
            if result is not None:
                return result
    return None
...
def process_data_update(ied_id, key, data):
    ...
    for config_path, ioa in mms_to_ioa_map.items():
        if ioa in valid_ioas_for_ied and config_path.startswith(mms_path_from_key):
            ...
            timestamp_ms = find_timestamp_ms(value_to_update)
            current_timestamp_str = datetime.fromtimestamp(timestamp_ms / 1000).strftime('%Y-%m-%d %H:%M:%S') if timestamp_ms else time.strftime('%Y-%m-%d %H:%M:%S')
            ...
            try:
                ...
                iec104_server.update_ioa(ioa, value_to_send, timestamp_ms)
...
async def main_async():
    ...
    logging.info(f"Gateway v18.0 (Rilis Final) dimulai. Tekan Ctrl+C untuk berhenti.")
