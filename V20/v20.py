...
# MODIFIED: Add ioa_to_signal_name_map
...
def process_data_update(ied_id, key, data):
    ...
    # --- MODIFIKASI DIMULAI: Dapatkan timestamp gateway ---
    gateway_timestamp_ms = time.time() * 1000
    # --- MODIFIKASI SELESAI ---
    ...
    for config_path, ioa in mms_to_ioa_map.items():
        if ioa in valid_ioas_for_ied and config_path.startswith(mms_path_from_key):
            ...
            try:
                ...
                # --- MODIFIKASI DIMULAI: Kirim timestamp ke server 104 ---
...
def do_invalidation(ied_id):
    if ied_id in ied_to_ioas_map:
        ...
        for ioa in ioas_to_invalidate:
            # MODIFIED: Add signal and ied_id to payload
            update_payload = {
                'type': 'invalidation',
                'ied_id': ied_id,
                'ioa': ioa,
...
async def main_async():
    ...
    for section in list(data_types.keys()) + list(command_types.keys()):
        if section in config:
            for ioa, config_line in config[section].items():
                # MODIFIED: New parsing logic for "SignalName:iec61850://..."
                signal_name = "N/A"
    ...
    logging.info(f"Gateway v20.0 (Rilis Final) dimulai. Tekan Ctrl+C untuk berhenti.")
...
# MODIFIED: Create a full index.html file with CSS and JS
