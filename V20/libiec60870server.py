...
class IEC60870_5_104_server:
    ...
    def update_ioa(self, ioa, data, timestamp_ms=None):
        value = 0
        io_type = self.IOA_list[ioa]['type']
        ...
        if value != self.IOA_list[ioa]['data']:
            ...
            if self.IOA_list[ioa]['event'] == True:
                ...
                if timestamp_ms is not None:
                    time = CP56Time2a()
                    CP56Time2a_setFromMsTimestamp(time, timestamp_ms)
                ...
                if io_type == MeasuredValueScaled:
                    ...
                    if timestamp_ms is not None:
                        CS101_ASDU_setTypeID(newAsdu, M_SP_TB_1)
                        io = cast(SinglePointWithCP56Time2a_create(None, ioa, self.IOA_list[ioa]['data'], IEC60870_QUALITY_GOOD, time),InformationObject)
                    else:
                        io = cast(SinglePointInformation_create(None, ioa, self.IOA_list[ioa]['data'], IEC60870_QUALITY_GOOD),InformationObject)
                elif io_type == DoublePointInformation:
                    if timestamp_ms is not None:
                        CS101_ASDU_setTypeID(newAsdu, M_DP_TB_1)
                        io = cast(DoublePointWithCP56Time2a_create(None, ioa, self.IOA_list[ioa]['data'], IEC60870_QUALITY_GOOD, time),InformationObject)
                    else:
