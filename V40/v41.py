#!/usr/bin/env python3
# v41.py - Rilis dengan Timestamp Millisecond (ms)
# Deskripsi: Gateway IEC 61850 ke IEC 60870-5-104.
# Fitur v41:
# - Menampilkan timestamp dengan presisi milidetik (ms) di Web UI dan Log.
# - Menambahkan kolom "Signal Name" di UI yang diambil dari file konfigurasi.
# - Arsitektur shutdown yang dirombak total menggunakan signal handler.
...
def process_data_update(ied_id, key, data):
    ...
    # Gunakan timestamp dari gateway/data, default ke waktu sekarang
        ...
        if ioa in valid_ioas_for_ied and config_path.startswith(mms_path_from_key):
            ...
            try:
                ...
                update_payload = {
                    ...
                    'value': value_to_send,
                    'timestamp': formatted_timestamp, # UPDATE: Timestamp sudah ada ms
                ...
                logging.info(f"[{ied_id}] Cocok '{reported_key}' -> IOA {ioa}, Val: {value_to_send}, Time: {formatted_timestamp}")
            except Exception as e:
...
def do_invalidation(ied_id):
    if ied_id in ied_to_ioas_map:
        ...
        formatted_timestamp = get_timestamp_with_ms() # UPDATE: Timestamp ms saat ini
...
async def main_async():
    ...
    logging.info(f"Gateway v41 dimulai. Tekan Ctrl+C untuk berhenti.")
...
def create_index_html_if_not_exists():
    if not os.path.exists("index.html"):
        logging.info("Membuat file index.html untuk v41...")
        html_content = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Gateway Status v41 (ms)</title>
    <style>
...
<body>
    <h1>Gateway Monitoring Status v41 (with ms)</h1>
