#!/usr/bin/env python3
# v100.py - Professional Edition: Carbon UI Final + Mode Tracking
# Deskripsi: Gateway IEC 61850 ke 104 dengan Dot Lamp, Status, & Mode Report/Polling.
            ...
            if(d.type === 'data_update' || d.type === 'invalidation') {{
                ...
                if(tb) {{
                    ...
                    if(!r) {{ 
                        ...
                        r.innerHTML = `<td class="font-mono">${{d.ioa}}</td><td>${{d.signal}}</td><td id="m-${{d.ioa}}"></td><td id="v-${{d.ioa}}" class="font-mono"></td><td id="stcol-${{d.ioa}}"></td><td class="font-mono" style="opacity:0.6">${{d.timestamp.split(' ')[1]}}</td>`; 
                    }}
