#!/usr/bin/env python3
# v100.py - Professional Edition: Carbon UI Final + Time Update Fix
# Deskripsi: Gateway IEC 61850 ke 104 dengan perbaikan sinkronisasi waktu di Tabel IED.
...
def process_data_update(ied_id, key, data, mode="Poll"):
    ...
    # Prioritas waktu: 1. Timestamp dari IED (Report), 2. Waktu Gateway (Polling)
        ...
        ws.onmessage = (e) => {{
            ...
            if(d.type === 'data_update' || d.type === 'invalidation') {{
                ...
                if(tb) {{
                    ...
                    // FIX: Tambahkan ID unik pada kolom waktu agar bisa diupdate
                    if(!r) {{ 
