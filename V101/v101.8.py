...
# Deskripsi: Gateway IEC 61850 ke 104 dengan sistem pencegahan spam log pada Live Events.
...
ied_last_state = {} # Pelacak status untuk memfilter tampilan event
...
async def ied_handler(ied_id, uris):
    ...
    while not shutdown_event.is_set():
        try:
            # Dashboard update (silent agar tidak spam event log)
            ...
            # HANYA panggil invalidasi jika sebelumnya status adalah CONNECTED (First Fail)
            if ied_last_state.get(ied_id) != "DISCONNECTED":
                ...
                # Dashboard tetap diupdate agar user tahu Gateway masih mencoba reconnect
        ...
        ws.onmessage = (e) => {{
            ...
            if(d.type === 'data_update' || d.type === 'invalidation') {{
                ...
                if(tb) {{
                    ...
                    document.getElementById('m-' + d.ioa).innerHTML = `<span class="badge ${{d.mode === 'Report' ? 'badge-blue' : 'badge-neutral'}}" style="font-size:9px;">${{d.mode}}</span>`;
