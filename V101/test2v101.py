#!/usr/bin/env python3
# v101.py - Professional Edition: Stable Gateway
# Deskripsi: Gateway IEC 61850 ke 104 dengan sinkronisasi thread & proteksi memori.

import faulthandler
faulthandler.enable()  # Mengaktifkan pelacakan jika terjadi crash di level C
...
def find_first_float(data):
    """Mencari angka pertama dalam struktur data MMS yang kompleks."""
...
def process_data_update(ied_id, key, data, mode="Poll"):
    ...
    for config_path, ioa in mms_to_ioa_map.items():
        if ioa in valid_ioas and config_path.startswith(mms_path):
            try:
                # Proteksi akses ke IOA_list menggunakan LOCK dari server
...
async def ied_handler(ied_id, uris):
    ...
    while not shutdown_event.is_set():
        try:
            ...
            while not shutdown_event.is_set():
                # Polling IED via executor agar tidak memblokir event loop
                await main_loop.run_in_executor(None, client.poll)
                
                # Cek status koneksi melalui pointer C
        ...
        ws.onmessage = (e) => {
            ...
            if(d.type === 'status_update') {
                ...
                if(!row) {
                    ...
                    p.innerHTML = `<div class="table-container"><table><thead><tr><th>IOA</th><th>Signal</th><th>Mode</th><th>Value</th><th>Status</th><th>Time</th></tr></thead><tbody id="tbody-${d.ied_id}"></tbody></table></div>`;
                    document.getElementById('ied-panes').appendChild(p);
                }
                document.getElementById('st-' + d.ied_id).innerHTML = `<span class="badge ${d.status === 'CONNECTED' ? 'badge-success' : 'badge-danger'}">${d.status}</span>`;
            }
            if(d.type === 'data_update' || d.type === 'invalidation') {
                ...
                if(tb) {
                    ...
                    if(!r) { r = tb.insertRow(); r.id = 'ioa-' + d.ioa; r.innerHTML = `<td>${d.ioa}</td><td>${d.signal}</td><td>${d.mode}</td><td id="v-${d.ioa}"></td><td id="stcol-${d.ioa}"></td><td id="tcol-${d.ioa}" style="opacity:0.6"></td>`; }
                ...
                if(d.is_event !== false) {
                    ...
                    er.innerHTML = `<td>${d.timestamp.split(' ')[1]}</td><td>${d.ied_id}</td><td>${d.ioa}</td><td>${d.signal}</td><td>${d.mode}</td><td>${d.value}</td><td><span class="badge">${d.value === 'INVALID' ? 'Link Fail' : d.value}</span></td>`;
