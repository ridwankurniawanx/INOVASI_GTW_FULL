#!/usr/bin/env python3
# v100.py - Professional Edition: Carbon UI Final
# Deskripsi: Gateway IEC 61850 ke 104 dengan Dot Lamp & Status Mapping.
...
# --- [LOGIC v100] Helper: Ekstraksi Nilai & Status ---
...
# --- [CORE v100] Data Processing Logic ---
        ...
        if ioa in valid_ioas and config_path.startswith(mms_path):
            try:
                ...
                payload = {
                    ...
                    'is_event': not is_meas, 'data_type': data_type
                }
                main_loop.call_soon_threadsafe(broadcast_queue.put_nowait, payload)

                if not is_meas:
                    event_logger.info(f"{fmt_ts} | {ied_id} | IOA: {ioa} | {ioa_to_signal_name_map.get(ioa)} | Val: {val_to_send} | {get_status_description(data_type, val_to_send)}")
...
# --- [UI] Professional Carbon Theme Generator ---
def create_index_html():
    logging.info("Generating UI Index (Professional Carbon Edition)...")
    html_content = f"""
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div id="header">
        ...
        <div style="display:flex; gap:10px; align-items:center;">
            <span style="font-size:11px; opacity:0.6;">By Ridwan</span>
        ...
        ws.onmessage = (e) => {{
            ...
            if(d.type === 'status_update') {{
                ...
                if(!row) {{
                    ...
                    row.innerHTML = `<td class="font-mono">${{d.ied_id}}</td><td id="st-${{d.ied_id}}"></td><td class="font-mono" style="opacity:0.6">${{d.timestamp}}</td>`;
                    ...
                    p.innerHTML = `<div class="table-container"><table><thead><tr><th>IOA</th><th>Signal Name</th><th>MMS Address</th><th>Value</th><th>Status</th><th>Time</th></tr></thead><tbody id="tbody-${{d.ied_id}}"></tbody></table></div>`;
                    document.getElementById('ied-panes').appendChild(p);
                    iedTabs[d.ied_id] = btn;
                }}
                const s = d.status.toLowerCase();
                const b = getBadgeClass(d.status);
                document.getElementById('st-' + d.ied_id).innerHTML = `<span class="badge ${{b}}">${{d.status}}</span>`;
                iedTabs[d.ied_id].querySelector('.status-dot').className = `status-dot ${{s}}`;
            }}
            ...
            if(d.type === 'data_update' || d.type === 'invalidation') {{
                ...
                if(tb) {{
                    ...
                    if(!r) {{ 
                        ...
                        r.innerHTML = `<td class="font-mono">${{d.ioa}}</td><td>${{d.signal}}</td><td class="font-mono" style="font-size:10px; opacity:0.5">${{d.address}}</td><td id="v-${{d.ioa}}" class="font-mono"></td><td id="stcol-${{d.ioa}}"></td><td class="font-mono" style="opacity:0.6">${{d.timestamp.split(' ')[1]}}</td>`; 
                    }}
                ...
                if(d.is_event !== false) {{
                    ...
                    er.innerHTML = `<td class="font-mono">${{d.timestamp.split(' ')[1]}}</td><td style="font-weight:600">${{d.ied_id}}</td><td class="font-mono">${{d.ioa}}</td><td>${{d.signal}}</td><td class="font-mono">${{d.value}}</td><td><span class="badge ${{getBadgeClass(st)}}">${{st}}</span></td>`;
