#!/usr/bin/env python3
# v100.py - Professional Edition: Carbon UI + Filtered Logs + MS Heartbeat
# Deskripsi: Gateway IEC 61850 ke 104 dengan sistem monitoring terfilter dan presisi tinggi.
...
def setup_logging():
    ...
    # Console & process.txt sekarang hanya mencatat log manual (Filtered)
...
# --- [CORE] Data Processing & Filtered Logging ---
    ...
    for config_path, ioa in mms_to_ioa_map.items():
        if ioa in valid_ioas and config_path.startswith(mms_path):
            try:
                ...
                # Filtered Log: Hanya mencatat sinyal yang terdaftar
                logging.info(f"[{mode}] {ied_id} | IOA: {ioa} | {sig_name} | Value: {val_to_send}")
...
async def ied_handler(ied_id, uris):
    while not shutdown_event.is_set():
        try:
            ...
            # loggerRef=None mematikan banjir log otomatis dari library
...
# --- [UI] Carbon Theme + MS Clock Generator ---
