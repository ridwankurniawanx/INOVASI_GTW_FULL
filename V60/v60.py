#!/usr/bin/env python3
# v60.py - Rilis UI Carbon Pro dengan Smart Auto-Save Report
# Deskripsi: Gateway IEC 61850 ke IEC 60870-5-104.
# Perbaikan: Otomatis menyimpan report yang ditemukan ke reports.ini secara permanen.
...
# --- Konfigurasi Nama File Log & Config ---
PROCESS_LOG_FILE = 'process.txt'
EVENT_LOG_FILE = 'events.txt'
REPORTS_PRIORITY_FILE = 'reports.ini'
...
# --- Smart RCB Management Logic (Fungsi Tambahan v60) ---

def auto_save_active_reports(client, ied_id):
    """Melihat apa yang berhasil diregistrasi library dan menyimpannya ke reports.ini"""
    if ied_id in client.reporting:
        active_rcbs = [entry["RPT"] for entry in client.reporting[ied_id]]
        if active_rcbs:
            cfg = configparser.ConfigParser()
            if os.path.exists(REPORTS_PRIORITY_FILE): cfg.read(REPORTS_PRIORITY_FILE)
            
            if not cfg.has_section(ied_id):
                cfg.add_section(ied_id)
                for i, rcb in enumerate(active_rcbs):
                    cfg.set(ied_id, f"Prio_{i+1}", rcb)
                with open(REPORTS_PRIORITY_FILE, 'w') as f: cfg.write(f)
                logging.info(f"[{ied_id}] Berhasil menyimpan {len(active_rcbs)} report aktif ke {REPORTS_PRIORITY_FILE}")

def try_enable_from_config(client, ied_id):
    """Mencoba mengaktifkan report yang sudah ada di reports.ini secara permanen"""
    cfg = configparser.ConfigParser()
    if os.path.exists(REPORTS_PRIORITY_FILE):
        cfg.read(REPORTS_PRIORITY_FILE)
        if cfg.has_section(ied_id):
            for key in cfg[ied_id]:
                rcb_path = cfg[ied_id][key]
                if client.registerForReporting(rcb_path, ied_id, rcb_path):
                    return rcb_path
    return None
...
async def ied_handler(ied_id, uris):
    ...
    while not shutdown_event.is_set():
        try:
            ...
            # --- FITUR v60: Smart Auto-Save Report ---
            # 1. Coba aktifkan dari config reports.ini dulu
            active_rcb = await main_loop.run_in_executor(None, try_enable_from_config, client, ied_id)
            
            # 2. Jika config kosong, lakukan registrasi biasa (Library akan mencari otomatis)
            if not active_rcb:
                count = await main_loop.run_in_executor(None, locked_register_values)
                # Simpan hasil temuan otomatis library ke reports.ini agar permanen
                await main_loop.run_in_executor(None, auto_save_active_reports, client, ied_id)
                # Ambil nama report yang aktif untuk status UI
                if ied_id in client.reporting and client.reporting[ied_id]:
                    active_rcb = client.reporting[ied_id][0]["RPT"]
            
            status_text = f"CONNECTED ({active_rcb if active_rcb else 'Polling Mode'})"
            broadcast_connection_status(ied_id, status_text)
...
async def main_async():
    ...
    for section in list(data_types.keys()) + list(command_types.keys()):
        if section in config:
            for ioa, config_line in config[section].items():
                signal_n, full_addr = "N/A", config_line.strip()
                if ':' in full_addr and '://' in full_addr:
                    parts = full_addr.split(':', 1)
                    if '://' in parts[1]: signal_n, full_addr = parts[0], parts[1]

                ioa_int = int(ioa)
                ioa_to_signal_name_map[ioa_int] = signal_n.strip()
    ...
    logging.info("Gateway v60 Berjalan. Tekan Ctrl+C untuk berhenti.")
...
async def shutdown(sig, loop):
    ...
    if 'iec104_server' in globals() and iec104_server: iec104_server.stop()
...
<body>
    ...
    <script>
        ...
        function updateIedStatus(iedId, status, timestamp) {
            ...
            if (status.includes('CONNECTED')) badge = 'badge-success';
            else if (status.includes('DISCONNECTED')) badge = 'badge-danger';
            else if (status.includes('CONNECTING')) badge = 'badge-warning';
        ...
        function addEventToTable(data, isNewUpdate) {
            ...
            if(data.data_type === 'SYSTEM') {
                badgeClass = (data.value.includes('CONNECTED')) ? 'badge-success' : 'badge-danger';
            }
