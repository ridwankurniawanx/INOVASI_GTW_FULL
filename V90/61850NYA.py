...
class iec61850client():
        ...
        def __init__(self, readvaluecallback = None, loggerRef = None, cmdTerm_cb = None, Rpt_cb = None):
                ...
                self.polling = {}
                self.last_poll_times = {} # Tambahan untuk throttling polling
                self.connections = {}
        ...
        def discovery(con):
                ...
                if error.value != 0:
                        logger.error("Gagal mendapatkan daftar logical device, error: %i" % error.value)
                        return {}
                if not deviceList:
                        return {}
                ...
                while device_item:
                    ld_name = ctypes.cast(lib61850.LinkedList_getData(device_item), ctypes.c_char_p).value.decode("utf-8")
                    ld_names.append(ld_name)
                    device_item = lib61850.LinkedList_getNext(device_item)
                
                lib61850.LinkedList_destroy(deviceList)

                ld_count = len(ld_names)
                if ld_count == 0:
                        return {}
                
                logger.info(f"Memulai discovery, ditemukan {ld_count} Logical Device(s).")
                processed_ld_count = 0

                for LD_name in ld_names:
                        processed_ld_count += 1
                        progress = (processed_ld_count / ld_count) * 100
                        
                        tmodel[LD_name] = {}
                        logicalNodes_c = lib61850.IedConnection_getLogicalDeviceDirectory(con, ctypes.byref(error), LD_name.encode('utf-8'))
                        
                        if error.value != 0:
                                sys.stdout.write(f"\rDiscovery Progress: {progress:.2f}% ({processed_ld_count}/{ld_count}) - Gagal membaca {LD_name}{' ' * 40}\n")
                                logger.error(f"Gagal mendapatkan direktori untuk LD {LD_name}, error: {error.value}")
                                continue
                        ...
                        ln_count = len(ln_names)
                        processed_ln_count = 0
                        
                        if ln_count == 0:
                                sys.stdout.write(f"\rDiscovery Progress: {progress:.2f}% ({processed_ld_count}/{ld_count}) - Membaca {LD_name} (Tidak ada LN){' ' * 20}")
                                sys.stdout.flush()

                        for LN_name in ln_names:
                                processed_ln_count += 1
                                sub_progress = (processed_ln_count / ln_count) * 100 if ln_count > 0 else 0
                                
                                progress_text = f"\rDiscovery: {progress:.1f}% ({processed_ld_count}/{ld_count}) | LD: {LD_name} -> LN: {sub_progress:.1f}% ({processed_ln_count}/{ln_count}) | Membaca {LN_name}..."
                                sys.stdout.write(progress_text + ' ' * (100 - len(progress_text)))
                                sys.stdout.flush()
                                ...
                                if error.value == 0 and LNobjects:
                                ...
                                if error.value == 0 and LNdss:
                                    ...
                                    while LNds:
                                            ...
                                            if error.value == 0 and dataSetMembers:
                                                ...
                                                while dataSetMemberRef:
                                                        ...
                                                        tmodel[LD_name][LN_name][DSname][str(i)] = {
                                                            'reftype': "DX", 'type': "reference", 'value': DX, 'FC': FC
                                                        }
                ...
                sys.stdout.write(f"\rDiscovery Selesai. Total {ld_count} Logical Device telah diproses.{' ' * 50}\n")
                sys.stdout.flush()
                return tmodel
                ...
                if reason != lib61850.IEC61850_REASON_NOT_INCLUDED:
                    ...
                    if mmsval:
                        ...
                        if submodel:
                            ...
                            if report_timestamp:
                                submodel["timestamp"] = report_timestamp
                            
                            if self.Rpt_cb:
                                self.Rpt_cb(DaRef, submodel)

        # --- FUNGSI ROBUST REPORTING (UNTOUCHED) ---
        def registerForReporting(self, key, tupl, ref_path):
                ...
                # 1. Cari Dataset yang mengandung ref_path yang diminta
                                        ...
                                        if isinstance(ds_content, dict) and ds_content.get("0", {}).get('reftype') == "DX":
                                                for index, dx_info in ds_content.items():
                                                        if ref_path.startswith(dx_info['value']):
                                                                logger.info(f"DATASET ditemukan! Ref:{ref_path} ada di DSRef: {LD_name}/{LN_name}.{DSname}")
                                                                target_ds_ref = f"{LD_name}/{LN_name}${DSname}"
                                                                target_ld = LD_name
                                                                target_ln = LN_name
                                                                target_ds_name = DSname
                                                                found_dataset = True
                                                                break
                
                if not found_dataset:
                        logger.error(f"RPT: Tidak dapat menemukan dataset untuk referensi: {ref_path}")
                        return False

                # 2. Cari SEMUA kandidat RCB (BRCB/URCB) yang dikonfigurasi menggunakan dataset ini
                candidates = []
                dos = model[target_ld][target_ln]
                for RP_name, rp_content in dos.items():
                        if isinstance(rp_content, dict) and rp_content.get("DatSet", {}).get("value") == target_ds_ref:
                                # Ambil Functional Constraint (FC) dari atribut DatSet (biasanya BR atau RP)
                                fc = rp_content.get("DatSet", {}).get("FC", "RP")
                                RPT_path = f"{target_ld}/{target_ln}.{fc}.{RP_name}"
                                candidates.append(RPT_path)
                
                if not candidates:
                        logger.error(f"RPT: Tidak ditemukan Report Control Block (RCB) untuk dataset {target_ds_ref}")
                        return False

                logger.info(f"Ditemukan {len(candidates)} kandidat RCB: {candidates}")

                # 3. Cek ketersediaan satu per satu (Iterasi Robust)
                for RPT_path in candidates:
                        # Cek apakah aplikasi ini sudah memakai RPT_path ini sebelumnya
                        if RPT_path in self.cb_refs:
                                logger.info(f"RPT {RPT_path} sudah diregistrasi oleh instance ini.")
                                return True

                        error = lib61850.IedClientError()
                        # Ambil status RCB real-time dari perangkat
                        rcb = lib61850.IedConnection_getRCBValues(con, ctypes.byref(error), RPT_path.encode('utf-8'), None)
                        
                        if error.value != 0:
                                logger.warning(f"Gagal mengambil nilai RCB {RPT_path}, error: {error.value}. Mencoba kandidat lain...")
                                continue

                        # Periksa apakah RCB sedang aktif (digunakan client lain)
                        is_enabled = lib61850.ClientReportControlBlock_getRptEna(rcb)
                        
                        if is_enabled:
                                logger.info(f"RPT {RPT_path} SEDANG DIGUNAKAN (Enabled=True). Mencoba urutan selanjutnya...")
                                lib61850.ClientReportControlBlock_destroy(rcb)
                                continue

                        # Jika RptEna == False, berarti RCB ini bebas digunakan
                        logger.info(f"RPT {RPT_path} TERSEDIA. Memulai proses registrasi...")
                        
                        RptId = lib61850.ClientReportControlBlock_getRptId(rcb)
                        cbh = lib61850.ReportCallbackFunction(self.ReportHandler_cb)
                        
                        refdata = [key, tupl, target_ld, target_ln, target_ds_name]
                        param_id = id(refdata)
                        
                        lib61850.IedConnection_installReportHandler(con, RPT_path.encode('utf-8'), RptId, cbh, param_id)

                        # Aktifkan Reporting dan General Interrogation (GI)
                        lib61850.ClientReportControlBlock_setRptEna(rcb, True)
                        lib61850.ClientReportControlBlock_setGI(rcb, True)
                        
                        lib61850.IedConnection_setRCBValues(con, ctypes.byref(error), rcb, 
                                                           lib61850.RCB_ELEMENT_RPT_ENA | lib61850.RCB_ELEMENT_GI, True)
                        ...
                        if error.value == 0:
                                ...
                                logger.info(f"Registrasi Reporting BERHASIL pada: {RPT_path}")
                                return True
                        else:
                                logger.error(f"Gagal mengaktifkan RCB {RPT_path}, error code: {error.value}")
                
                logger.error(f"RPT: Semua kandidat RCB untuk {ref_path} sedang sibuk atau tidak dapat diakses.")
                return False
        ...
        # --- FUNGSI POLLING DENGAN THROTTLE 5 DETIK ---
        def poll(self):
                now = time.time()
                for key, val in list(self.polling.items()):
                        # Cek apakah sudah 5 detik sejak polling terakhir untuk key ini
                        if now - self.last_poll_times.get(key, 0) < 5:
                                continue
                        ...
                        if self.getIED(uri_ref.hostname, port) == 0:
                                ...
                                model = self.connections[tupl]['model']
                                if con and model:
                                        # Update timestamp sebelum melakukan request ke IED
...
if __name__=="__main__":
        ...
        # Callback sederhana untuk melihat perubahan nilai
        def my_callback(ref, submodel):
            print(f"Update [{ref}]: {submodel.get('value')}")

        cl = iec61850client(readvaluecallback=my_callback)
        model = cl.getDatamodel(hostname=hostname, port=tcpPort)
        
        # Contoh registrasi nilai untuk dipantau
        # Secara otomatis akan mencoba Reporting dulu, jika gagal baru masuk ke daftar Polling
        test_ref = f"iec61850://{hostname}:{tcpPort}/IED1_XCBRGenericIO/CSWI1.Pos.stVal[ST]"
        cl.registerReadValue(test_ref)

        print("Memasuki Main Loop (Polling ditangani setiap 5 detik secara internal)...")
        try:
            while True:
                cl.poll()  # Aman dipanggil secepat mungkin, internal akan membatasi ke 5 detik
                time.sleep(0.1) # Untuk mencegah penggunaan CPU 100%
